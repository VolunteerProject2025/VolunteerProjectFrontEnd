...
const ChatBox = () => {
  ...
  // Only fetch recipient user for direct chats, not group chats
  const { recipientUser } = useFetchRecipientUser(
    currentChat?.isGroupChat ? null : currentChat,
    user
  );
  ...
  // Fetch group members info for displaying names
  useEffect(() => {
    if (!isGroupChat || !currentChat?.members || !messages) return;
    
    const uniqueSenderIds = [...new Set(
      messages.map(msg => msg.senderId)
    )].filter(id => !groupMembers[id] && id !== user._id);
    
    // If no new senders to fetch, exit early
    ...
    const fetchMembers = async () => {
      ...
      for (const senderId of uniqueSenderIds) {
        const userData = await fetchUserById(senderId);
        if (userData) {
          newMembersData[senderId] = userData;
        }
      }
      
      // Only update state if we have new data
      if (Object.keys(newMembersData).length > 0) {
        setGroupMembers(prev => ({...prev, ...newMembersData}));
      }
  ...
  }, [isGroupChat, currentChat, messages, fetchUserById, user._id]);
  ...
  const getSenderInfo = (senderId) => {
    ...
    // For group chats, use the cached member data
    if (isGroupChat) {
      const memberData = groupMembers[senderId];
      if (memberData) {
        return {
          name: memberData.fullName || `User ${senderId.substring(0, 4)}`,
          initials: getInitials(memberData.fullName)
        };
      }
      
      // Better fallback while loading
      return {
        name: loadingMembers ? "Loading..." : `Group Member`,
        initials: "?"
      };
    }
              ...
              >
                ...
                <div className={`card ${isCurrentUser ? 'bg-primary text-white' : ''}`}>
                  <div className="card-header d-flex justify-content-between p-2">
                    ...
                    <p className={`${isCurrentUser ? 'text-white' : 'text-muted'} small mb-0`}>
